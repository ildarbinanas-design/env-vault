...
// MatrixProof is the sole durable boundary between deep report validation and
// baseline comparison. The E2E runner validates every raw report exactly once,
// then seals the normalized evidence below. Baseline commands intentionally do
// not accept or inspect raw report directories.
...
// SealPlatformProof binds every normalized field and every raw-evidence digest
// into one stable digest. It must be called only after deep validation passes.
...
// LoadMatrixProof performs strict schema and release-contract validation. It
// returns a decoded proof alongside validation errors so callers can still
// emit fail-closed machine evidence for malformed input.
func LoadMatrixProof(filename string, contract releasecontract.Contract) (MatrixProof, error) {
	...
	if info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() || info.Size() <= 0 || info.Size() > 16<<20 {
		return MatrixProof{}, errors.New("matrix proof must be a non-empty regular file no larger than 16 MiB")
	}
...
func (proof MatrixProof) Validate(contract releasecontract.Contract) error {
	...
	for _, check := range proof.Checks {
		if check.Name == "" || check.Status != "pass" || check.Detail != "" {
			add("matrix check %q did not pass cleanly", check.Name)
		}
	}
	...
	for index, declared := range contract.Platforms {
		...
		if !goPattern.MatchString(actual.GoVersion) || !reporterPattern.MatchString(actual.GotestsumVersion) || actual.SubjectKind != "artifact" || !validSHA256(actual.BinarySHA256) || !validSHA256(actual.ContractSHA256) {
			add("platform %s toolchain, subject, binary, or contract identity is invalid", declared.ID)
		}
		if index == 0 {
		...
		for _, scenario := range actual.CriticalScenarios {
		...
		if !sort.SliceIsSorted(actual.CriticalScenarios, func(left, right int) bool {
			return actual.CriticalScenarios[left].ID < actual.CriticalScenarios[right].ID
		}) {
