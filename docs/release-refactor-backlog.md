...
## 14. DevSecOps-aligned provenance/SBOM in an isolated build contour — `frozen; deferred by design, see docs/pipeline-complexity-proposal-2026-07-30.md`

- **Problem and evidence:** `docs/pipeline-complexity-proposal-2026-07-30.md`
  (2026-07-30) recommended trimming the current bespoke provenance/SBOM
  generation entirely, since no consumer verifies it and it added custom
  code to the same repo-controlled workflow context it's meant to attest to
  — which weakens the actual security property (a builder that can be
  influenced by repo-controlled code cannot produce non-forgeable
  provenance about that same build). The owner (2026-07-30) asked to keep
  the *capability* on the backlog, but reworked to industry-standard
  practice instead of bespoke code, once trim (item above, in progress) is
  done.
- **Affected files/workflows:** whatever remains of the current custom
  provenance/SBOM generation is removed by the trim work; this item
  describes its eventual, differently-built replacement, not a modification
  of the removed code.
- **Guarantee preserved (if built):** an artifact's provenance is
  attributable to an exact source commit and build recipe, and is
  verifiable by a third party without trusting the repository maintainer's
  say-so — the actual SLSA threat model, only worth building if `env-vault`
  ever has a consumer besides the owner.
- **Proposed architecture (DevSecOps best practice, not bespoke code):**
  - **Provenance**: GitHub's native `actions/attest-build-provenance` (or
    the SLSA GitHub generator for SLSA Build L3) run from a separate,
    minimally-privileged reusable workflow — not the same job that produces
    the binaries — so the attestation is generated by infrastructure the
    release workflow's own code cannot influence. This replaces the current
    hand-rolled provenance/attestation code with a maintained, widely-
    audited GitHub Actions component; near-zero bespoke Go/shell.
  - **SBOM**: generate with a standard tool (`anchore/sbom-action` /
    `syft`, or `cyclonedx-gomod` for the Go module graph) in CycloneDX or
    SPDX format, attached to the Release as a signed asset via the same
    native attestation action — not a custom SBOM schema/generator.
  - **Verification**: consumers (including the owner, for personal
    due-diligence) verify with `gh attestation verify <artifact> -R
    ildarbinanas-design/env-vault` — GitHub's built-in verification against
    its Sigstore-backed attestation registry. No bespoke verification CLI
    or offline-replay ledger needed; this is the direct DevSecOps-standard
    replacement for the current versioned evidence-ledger's "prove what was
    published" role, at a fraction of the code.
- **Expected reduction vs. the pre-trim implementation:** near-total removal
  of bespoke provenance/evidence Go code, replaced by a few workflow steps
  calling maintained third-party actions; no custom schema versioning to
  maintain.
- **Risk:** a misconfigured reusable-workflow boundary (e.g. attestation job
  sharing secrets/permissions with the build job) reintroduces the
  forgeability problem this exists to solve — the isolation between builder
  and attestor is the entire point, not an implementation detail.
- **Required tests:** attestation verifies against the exact published
  artifact digest; a tampered/re-uploaded artifact fails verification; SBOM
  component list matches `go.mod`/`go.sum` at the release commit; the
  attestation job has no write access to the release/tag itself.
- **Dependencies and order:** only after the trim work above removes the
  current custom provenance/SBOM/evidence-ledger code — this is a from-
  scratch rebuild on a different architecture, not a migration of the old
  code. No PersonalOS consumer or security requirement currently triggers
  starting it (ADR 0008 gating applies); build it if/when `env-vault` gets
  a second real consumer, or as a low-cost hygiene pass once trim settles.
- **Acceptance criteria:** `gh attestation verify` succeeds against a real
  published release artifact; SBOM asset is present and accurate on the
  Release; zero bespoke Go packages are needed to produce or verify either.
