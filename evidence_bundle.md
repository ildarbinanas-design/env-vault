...
> **Historical read-only archive.** Do not append to this narrative. New
> implementation and release evidence is authoritative only as versioned
> `evidence/*.evidence.json`, with `evidence/README.md` generated by
> `go run ./cmd/release-evidence implementation` or `index`.
