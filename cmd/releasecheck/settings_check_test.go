...
func TestSettingsCheckCLIValidatesSavedResponsesOffline(t *testing.T) {
	...
	if !result.OK || result.SchemaID != releasesettings.CheckSchemaID || result.SchemaVersion != releasesettings.SchemaVersion || result.Repository != "ildarbinanas-design/env-vault" || result.Result != releasesettings.ResultPass {
...
func settingsCheckArgs(t *testing.T, inputs releasesettings.RawInputs) []string {
	t.Helper()
	return []string{
		"settings", "check", "--contract", canonicalContractPath(t), "--repository", "ildarbinanas-design/env-vault",
...
func validSettingsCheckInputs() releasesettings.RawInputs {
	...
	return releasesettings.RawInputs{
		...
		TagRuleset:      []byte(`{"id":8,"name":"Protect env-vault release tags","target":"tag","source_type":"Repository","source":"ildarbinanas-design/env-vault","enforcement":"active","current_user_can_bypass":"never","conditions":{"ref_name":{"include":["refs/tags/v*"],"exclude":[]}},"rules":[{"type":"deletion"},{"type":"update"}]}`),
		EvidenceRuleset: []byte(`{"id":9,"name":"Protect env-vault release evidence","target":"branch","source_type":"Repository","source":"ildarbinanas-design/env-vault","enforcement":"active","current_user_can_bypass":"never","conditions":{"ref_name":{"include":["refs/heads/release-evidence"],"exclude":[]}},"rules":[{"type":"deletion"},{"type":"non_fast_forward"}]}`),
	}
}
